...
data_name_array=("${data_train}")
data_list_path_array=("${data}/${data_train}/${data_type}.list")
data_scp_path_array=("${data}/${data_train}/wav.scp")
data_lmdb_path_array=("${data}/${data_train}/lmdb")
nj_array=($nj)
batch_size_array=(4)
num_workers_array=(3)
exp_subdir=("${subdir}")
...
for i in $(seq 0 $(($count - 1))); do
  ...
  echo "[INFO] Extracting embeddings for ${data_name_array[$i]} with $wavs_num utterances..."
